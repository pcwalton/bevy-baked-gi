...
use bevy::prelude::{
    ...
    Or, Plugin, PostUpdate, Query, Rect, Res, ResMut, Resource, StandardMaterial, Update, Vec2,
    Vec3, Vec4, With, Without, World,
};
...
#[derive(Clone, Component, ExtractComponent, AsBindGroup, Reflect)]
pub struct ComputedLightmapInfo {
    #[texture(0)]
    #[sampler(1)]
...
type DrawIrradianceVolumePbrMaterial = (
    SetItemPipeline,
...
type DrawLightmappedPbrMaterial = (
    SetItemPipeline,
    SetMeshViewBindGroup<0>,
...
impl Plugin for IrradianceVolumesPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.register_type::<IrradianceVolume>()
            ...
            .add_plugins(ExtractComponentPlugin::<Handle<IrradianceVolumePbrMaterial>>::extract_visible())
        ...
        render_app
            ...
            .add_systems(ExtractSchedule, pbr::extract_materials::<IrradianceVolumePbrMaterial>)
            .add_systems(
                Render,
                (
                    ...
                    pbr::queue_shadows::<IrradianceVolumePbrMaterial>.in_set(RenderLightSystems::QueueShadows),
...
fn compute_global_illumination(
    ...
    mut entities_query: Query<(Entity, Option<&Lightmap>), With<Handle<IrradianceVolumePbrMaterial>>>,
) {
    // FIXME: Check distance, fill in appropriately.

    for (entity, maybe_lightmap) in entities_query.iter_mut() {
        /*
        commands.entity(entity).insert(ComputedIrradianceVolumeInfo {
            ...
            lightmap: maybe_lightmap.map(|lightmap| lightmap.image.clone()),
            lightmap_uv_rect: match maybe_lightmap {
...
pub fn queue_pbr_gi_material_meshes(
    ...
    material_meshes: Query<(&Handle<IrradianceVolumePbrMaterial>, &Handle<Mesh>, &MeshUniform)>,
    images: Res<RenderAssets<Image>>,
    mut views: Query<(
        &ExtractedView,
        &VisibleEntities,
    ...
    {
        let draw_opaque_pbr = opaque_draw_functions.read().id::<DrawIrradianceVolumePbrMaterial>();
        let draw_alpha_mask_pbr = alpha_mask_draw_functions.read().id::<DrawIrradianceVolumePbrMaterial>();
        let draw_transparent_pbr = transparent_draw_functions.read().id::<DrawIrradianceVolumePbrMaterial>();
...
pub fn instantiate_lightmaps_in_gltf(
    ...
    for (entity, children, gltf_extras) in gltf_extras_query.iter_mut() {
        ...
        let lightmap_rect = lightmap_coords.map(|coords| {
            Rect::from_corners(vec2(coords[0], coords[1]), vec2(coords[2], coords[3]))
        });
            ...
            */
            ...
            commands
                ...
                .insert(Lightmap {
                    ...
                    uv_rect: lightmap_rect.unwrap_or(Rect::from_corners(Vec2::ZERO, Vec2::ONE)),
                });
